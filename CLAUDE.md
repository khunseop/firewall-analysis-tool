...
| 분석 엔진 | `app/services/analysis/` | 7개 비동기 엔진 (`redundancy`, `unused`, `impact`, `unreferenced_objects`, `risky_ports`, `over_permissive`, `unused_ng_policy`). 백그라운드 태스크는 자체 `SessionLocal()` 세션을 열고, CPU 비교 연산은 `CPU_EXECUTOR`에서 실행. `analysistasks` 테이블로 진행률 추적, 결과는 JSON 저장. `unused_ng_policy`만 예외적으로 사용이력 수집을 위해 장비에 라이브 접속(`IO_EXECUTOR`) — 나머지는 DB 조회만 함 |
...
- **새 분석 모듈 추가(프로젝트형, deletion_workflow 참고)**: `models/analysis.py`의 `AnalysisTaskType`에 항목 추가, `AnalysisProject.module_type`에 새 값 사용(테이블/스키마 변경 불필요 — `analysis_projects`/`analysis_project_files`를 그대로 공유) → 모듈 전용 서비스 패키지(`app/services/<module>/`)에 파이프라인 로직 + `tasks.py`(백그라운드 실행, `services/deletion_workflow/tasks.py` 패턴 복제) 작성 → 모듈 전용 실행 엔드포인트(`endpoints/<module>.py`) 추가(프로젝트 CRUD는 공용 `endpoints/analysis_projects.py` 재사용, 라우트 추가 불필요) → `crud/crud_analysis.py`의 `PROJECT_MODULE_TASK_TYPES` 집합에 새 `task_type` 값 추가(누락 시 파이프라인 단계 실행이 quick 분석 이력 목록에 섞여 노출됨) → 프론트 `analysis-modules/`에 `kind: 'project'` 모듈 파일 추가 후 레지스트리 등록 → `App.tsx`에 해당 모듈의 상세/위저드 라우트(`analysis/projects/<module_type>/:id`)를 직접 추가(목록 라우트 `analysis/projects/:moduleType`는 이미 범용이라 추가 불필요하지만, `ProjectAnalysisModule` 타입에 `detailRoute`가 없어 상세 라우트는 레지스트리에서 자동 유도되지 않음) → 위저드 UI는 모듈 전용 컴포넌트로 직접 작성(공통화되어 있지 않음).
