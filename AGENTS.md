# FPAT Agent: FIREWALL POLICY ANALYSIS TOOL

## 1. 프로젝트 개요

**FPAT(Firewall Policy Analysis Tool)**은 여러 방화벽 장비의 정책을 통합 조회하고 분석할 수 있는 **웹 기반 정책 관리 도구**입니다.  
본 문서는 FPAT의 **Agent 구성요소**에 대한 명세를 정의하며, 주로 **정책 데이터 수집 및 동기화 로직**을 다룹니다.  
...
- **세부 기능:**
  - 사용자가 출발지, 목적지, 서비스(포트)를 입력하면, FPAT이 저장된 정책 테이블을 기반으로  
...
> 📘 본 문서는 FPAT Agent의 구현 및 유지보수 기준 문서입니다.  
