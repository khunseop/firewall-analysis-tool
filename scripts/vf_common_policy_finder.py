...
사용법:
    ...
    python scripts/vf_common_policy_finder.py file1.xlsx file2.xlsx ...
        → 지정한 파일들로 바로 실행 (기본 제외 컬럼: 통보대상 파일 기준)

    python scripts/vf_common_policy_finder.py --exclude "No,Type,Rule Name" file1.xlsx file2.xlsx ...
        → 공통기준에서 제외할 컬럼을 직접 지정 (예: 중복정책 파일)

    python scripts/vf_common_policy_finder.py --header-row 0 file1.xlsx file2.xlsx ...
        → 헤더가 1행부터 시작하는 파일(제목행 없음)인 경우
...
# 기본값: 통보대상 파일은 1행이 제목 등 부가정보이고 헤더가 2행부터 시작 (0-indexed)
DEFAULT_HEADER_ROW = 1

# 기본값: 통보대상 파일 기준 공통정책 판단에서 제외할 컬럼 — 원본/번역본 컬럼명을 모두 포함
# (--exclude로 파일 종류에 맞게 덮어쓸 수 있음. 예: 중복정책 파일 → "No,Type,Rule Name")
DEFAULT_ID_COLUMNS = {"Rule Name", "규칙명", "Seq", "순번", "No", "No.", "Description", "설명"}
...
def load_file(path: Path, header_row: int) -> pd.DataFrame:
...
def parse_args():
    ...
    parser.add_argument("files", nargs="*", help="비교할 xlsx 파일들 (생략 시 현재 디렉터리에서 선택)")
    parser.add_argument(
        "--exclude", default=None,
        help='공통기준에서 제외할 컬럼명, 콤마로 구분 (예: "No,Type,Rule Name"). 생략 시 통보대상 파일 기본값 사용',
    )
    parser.add_argument(
        "--header-row", type=int, default=DEFAULT_HEADER_ROW,
        help=f"헤더가 있는 행 번호, 0-indexed (기본값 {DEFAULT_HEADER_ROW} = 2행)",
    )
...
def main():
    args = parse_args()
    files = [Path(a) for a in args.files] if args.files else select_files_in_cwd()
    id_columns = (
        {c.strip() for c in args.exclude.split(",") if c.strip()}
        if args.exclude is not None
        else DEFAULT_ID_COLUMNS
    )
    ...
    dataframes = {f.name: load_file(f, args.header_row) for f in files}
