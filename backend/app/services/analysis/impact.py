...
class ImpactAnalyzer:
    ...
    async def _get_policies_with_members(self) -> List[Policy]:
        """
        분석에 필요한 정책 및 관련 멤버(주소, 서비스) 데이터를 조회합니다 (common 모듈로 위임).
        """
        logger.info("정책이동 영향분석 대상 정책 데이터 조회 시작...")
        policies = await load_active_policies_with_members(self.db, self.device_id)
    ...
    def _get_policy_ranges(self, policy: Policy) -> Tuple[Set[Tuple[int, int]], Set[Tuple[int, int]], Set[Tuple[str, int, int]]]:
        """정책에서 IP 및 서비스(포트) 범위를 추출합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.get_policy_ranges(policy)

    def _ranges_overlap(self, range1: Tuple[int, int], range2: Tuple[int, int]) -> bool:
        """두 수치 범위(IP 또는 Port)가 서로 겹치는지 확인합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.ranges_overlap(range1, range2)

    def _services_overlap(self, svc1: Tuple[str, int, int], svc2: Tuple[str, int, int]) -> bool:
        """두 서비스 항목이 서로 겹치는지 확인합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.services_overlap(svc1, svc2)

    def _policies_overlap(self, policy1: Policy, policy2: Policy) -> bool:
        """두 정책의 조건이 모두 중첩되는지 확인합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.policies_overlap(policy1, policy2)

    def _applications_overlap(self, app1: str, app2: str) -> bool:
        """두 애플리케이션 목록이 서로 겹치는지 확인합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.applications_overlap(app1, app2)

    def _get_policy_members(self, policy: Policy) -> Tuple[List, List, List]:
        """정책의 출발지/목적지/서비스 멤버 레코드를 방향별로 분류하여 반환합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.get_policy_members(policy)

    def _ranges_overlap_any(self, ranges1: List[Tuple[int, int]], ranges2: List[Tuple[int, int]]) -> bool:
        """멤버 리스트 중 하나라도 서로 겹치면 True (policy_overlap 모듈로 위임)."""
        return policy_overlap.ranges_overlap_any(ranges1, ranges2)

    def _services_overlap_any(self, svc1: List[Tuple[str, int, int]], svc2: List[Tuple[str, int, int]]) -> bool:
        """서비스 멤버 리스트 중 하나라도 서로 겹치면 True (policy_overlap 모듈로 위임)."""
        return policy_overlap.services_overlap_any(svc1, svc2)
    ...
    def _member_ranges_overlap(
        ...
        """주어진 (가상의) 출발지/목적지/서비스/애플리케이션 조건이 policy2와 겹치는지 확인합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.member_ranges_overlap(src1, dst1, svc1, app1, policy2)

    def _overlap_details(self, policy1: Policy, policy2: Policy) -> Dict[str, List[Tuple[Any, Any]]]:
        """두 정책 간 실제로 교집합이 발생한 멤버 쌍을 카테고리별로 반환합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.overlap_details(policy1, policy2)

    @staticmethod
    def _format_ip_range(start: int, end: int) -> str:
        """IP 정수 범위를 사람이 읽을 수 있는 문자열로 변환합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.format_ip_range(start, end)

    @staticmethod
    def _format_service(member: "PolicyServiceMember") -> str:
        """서비스 멤버를 사람이 읽을 수 있는 문자열로 변환합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.format_service(member)

    def _describe_overlap(self, details: Dict[str, List[Tuple[Any, Any]]]) -> str:
        """겹치는 구체적인 값을 사람이 읽을 수 있는 문장으로 요약합니다 (policy_overlap 모듈로 위임)."""
        return policy_overlap.describe_overlap(details)
