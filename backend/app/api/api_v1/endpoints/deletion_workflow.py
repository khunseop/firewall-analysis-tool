...
fpat policy_deletion_processor의 태스크들을 REST API로 노출합니다.
DB→Excel 변환은 services/deletion_workflow/export_service.py,
설정 연동은 config_bridge.py, 태스크 메타는 task_meta.py에 위임합니다.
"""
...
async def list_tasks():
    ...
    return {
        ...
        "fpat_yaml": fpat_yaml_path() or "설정 파일 없음 — fpat/fpat.yaml 확인 필요",
    }
...
async def extract_device_data(
    ...
    try:
        content, filename = await build_device_export(db, device)
...
async def complete_project(
    ...
    # 중복정책 정리/공지/삭제: Task 15(예외처리 후) 우선, 없으면 Task 14
    dup_task = 15 if any(files_map.get((15, s)) for s in ("output_0", "output_1", "output_2")) else 14
