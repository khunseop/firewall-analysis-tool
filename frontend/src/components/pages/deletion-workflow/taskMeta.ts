...
export interface TaskMeta {
  ...
  name: string
  description: string
  externalInputs?: { slot: string; label: string; required: boolean; hint?: string }[]
  autoFromDb?: boolean
}
...
export const PHASE1_TASKS: TaskMeta[] = [
  ...
  { step: 4, id: 5,  name: 'MIS ID 매핑',        description: '정책 파일 + MIS CSV → MIS ID 추가 (선택)',
    externalInputs: [{ slot: 'external_1', label: 'MIS CSV 파일', required: false,
      hint: '필요 컬럼: ruleset_id, mis_id (CSV, UTF-8 또는 CP949 인코딩)' }] },
...
export const PHASE2_TASKS: TaskMeta[] = [
  ...
  { step: 5, id: 12, name: '사용이력 반영',           description: '예외처리 결과 + 히트카운트 → 사용이력 반영',
    externalInputs: [{ slot: 'external_1', label: '사용이력 파일', required: false,
      hint: '필요 컬럼: Rule Name(필수) + 미사용여부 또는 Unused Days. Policies 페이지의 "사용이력" 내보내기 파일을 그대로 사용 가능합니다.' }] },
  ...
  { step: 11, id: 18, name: '공지대상 분류',          description: '정책 Excel → 공지대상 컬럼 추가(최종본)' },
]
